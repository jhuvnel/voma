function varargout = seg_data_v2(varargin)
% SEG_DATA_V2 MATLAB code for seg_data_v2.fig
%      SEG_DATA_V2, by itself, creates a new SEG_DATA_V2 or raises the existing
%      singleton*.
%
%      H = SEG_DATA_V2 returns the handle to a new SEG_DATA_V2 or the handle to
%      the existing singleton*.
%
%      SEG_DATA_V2('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in SEG_DATA_V2.M with the given input arguments.
%
%      SEG_DATA_V2('Property','Value',...) creates a new SEG_DATA_V2 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before seg_data_v2_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to seg_data_v2_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help seg_data_v2

% Last Modified by GUIDE v2.5 26-Sep-2016 17:33:57

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @seg_data_v2_OpeningFcn, ...
                   'gui_OutputFcn',  @seg_data_v2_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before seg_data_v2 is made visible.
function seg_data_v2_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to seg_data_v2 (see VARARGIN)

% Choose default command line output for seg_data_v2
...
% UIWAIT makes seg_data_v2 wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = seg_data_v2_OutputFcn(hObject, eventdata, handles) 
...
%
t1_cutout_eye = find(handles.Data.Time_Eye >= time_cutout_s(1,1));
t2_cutout_eye = find(handles.Data.Time_Eye >= time_cutout_s(2,1));
i_start_eye = t1_cutout_eye(1);
i_end_eye = t2_cutout_eye(1);

t1_cutout_head = find(handles.Data.Time_Head >= time_cutout_s(1,1));
t2_cutout_head = find(handles.Data.Time_Head >= time_cutout_s(2,1));
...
Segment.Time_Head = handles.Data.Time_Head(i_start_head:i_end_head);

Segment.HeadMPUVel_X = handles.Data.HeadMPUVel_X(i_start_head:i_end_head);
Segment.HeadMPUVel_Y = handles.Data.HeadMPUVel_Y(i_start_head:i_end_head);
Segment.HeadMPUVel_Z = handles.Data.HeadMPUVel_Z(i_start_head:i_end_head);

Segment.HeadMPUAccel_X = handles.Data.HeadMPUAccel_X(i_start_head:i_end_head);
Segment.HeadMPUAccel_Y = handles.Data.HeadMPUAccel_Y(i_start_head:i_end_head);
Segment.HeadMPUAccel_Z = handles.Data.HeadMPUAccel_Z(i_start_head:i_end_head);

Segment.Stim_Trig = handles.Data.Stim_Trig(i_start_eye:i_end_eye);

Segment.fname = handles.params.segment_filename;
...
%     plot(handles.data_plot,Segment.Time_Head,Segment.HeadMPU_X,'color',[1 0.65 0],'LineStyle',':','DisplayName','MPU-X')
%     plot(handles.data_plot,Segment.Time_Head,Segment.HeadMPU_Y,'color',[0.55 0.27 0.07],'LineStyle',':','DisplayName','MPU-Y')
%     plot(handles.data_plot,Segment.Time_Head,Segment.HeadMPU_Z,'color','r','LineStyle',':','DisplayName','MPU-Z')
% end
% 
...
if handles.params.plot_MPUData == 1
    plot(handles.data_plot,handles.Segment.Time_Head,handles.Segment.HeadMPUVel_X,'color',[1 0.65 0],'LineStyle',':','DisplayName','MPU-X')
    plot(handles.data_plot,handles.Segment.Time_Head,handles.Segment.HeadMPUVel_Y,'color',[0.55 0.27 0.07],'LineStyle',':','DisplayName','MPU-Y')
    plot(handles.data_plot,handles.Segment.Time_Head,handles.Segment.HeadMPUVel_Z,'color','r','LineStyle',':','DisplayName','MPU-Z')
end
...
if handles.params.plot_REData == 1
    plot(handles.data_plot,handles.Segment.Time_Eye,handles.Segment.RE_Position_X,'color',[255 165 0]/255,'DisplayName','LE-X')
    plot(handles.data_plot,handles.Segment.Time_Eye,handles.Segment.RE_Position_Y,'color',[138 43 226]/255,'DisplayName','LE-Y')
    plot(handles.data_plot,handles.Segment.Time_Eye,handles.Segment.RE_Position_Z,'color',[255,0,255]/255,'DisplayName','LE-Z')
end
...
switch handles.params.system_code
    ...
    case 1 % Labyrinth Devices 3D VOG
        ...
        switch handles.params.goggleID
            ...
            case {1}
                XvelHeadOffset = mean(data(1:50,XvelHeadIndex));
                YvelHeadOffset = mean(data(1:50,YvelHeadIndex));
                ZvelHeadOffset = mean(data(1:50,ZvelHeadIndex));
        ...
        XaccelHeadOffset = mean(data(1:50,XaccelHeadIndex));
        YaccelHeadOffset = mean(data(1:50,YaccelHeadIndex));
        ZaccelHeadOffset = mean(data(1:50,ZaccelHeadIndex));
        
        XvelHead = data(1:length(Time_Eye),XvelHeadIndex)*gyroscale - XvelHeadOffset;
        YvelHead = data(1:length(Time_Eye),YvelHeadIndex)*gyroscale - YvelHeadOffset;
        ZvelHead = data(1:length(Time_Eye),ZvelHeadIndex)*gyroscale - ZvelHeadOffset;
...
%         ZvelHead = data(1:length(Time_Eye),28)*accelscale - ZvelHeadOffset;
        ...
        XaccelHead = data(1:length(Time_Eye),XaccelHeadIndex)*accelscale;
        YaccelHead = data(1:length(Time_Eye),YaccelHeadIndex)*accelscale;
        ZaccelHead = data(1:length(Time_Eye),ZaccelHeadIndex)*accelscale;
        ...
        B = Rotation_Head' * [XvelHead' ; YvelHead' ; ZvelHead'];
        ...
        Time_Head = Time_Eye - Head_Sensor_Latency;
...
%         Data.Stim = Stim;
        ...
        Data.RE_Velocity_X = EyeVel.lx;
        Data.RE_Velocity_Y = EyeVel.ly;
        Data.RE_Velocity_LARP = EyeVel.ll;
        Data.RE_Velocity_RALP = EyeVel.lr;
        Data.RE_Velocity_Z = EyeVel.lz;
...
%         
%         if handles.params.plot_MPUData == 1
%             plot(handles.data_plot,Data.Time_Head,Data.HeadMPU_X,'color',[1 0.65 0],'LineStyle',':','DisplayName','MPU-X')
%             plot(handles.data_plot,Data.Time_Head,Data.HeadMPU_Y,'color',[0.55 0.27 0.07],'LineStyle',':','DisplayName','MPU-Y')
%             plot(handles.data_plot,Data.Time_Head,Data.HeadMPU_Z,'color','r','LineStyle',':','DisplayName','MPU-Z')
%         end
%         
%         if handles.params.plot_MVIGPIO == 1
...
%         hold on
%         plot(handles.data_plot,Time_Head,Axis2HeadVel,'color',[0.55 0.27 0.07])
%         plot(handles.data_plot,Time_Head,Axis3HeadVel,'color','r')
%         plot(handles.data_plot,Time_Head,Stim,'color','k')
%         
%         xlabel('Time [s]')
%         ylabel('stimulus Amplitude')
%         

...
%                             end
                        ...
                        switch stimnum{1}
                            ...
                            case handles.canalInfo.stimNum{3}
                                ...
                                %%%% Meg change *******
                                case handles.canalInfo.stimNum{4}
                                setappdata(handles.stim_axis,'ax',handles.canalInfo.stimCanal{4});
                                handles.stim_axis.String = handles.canalInfo.stimCanal(4);
                                setappdata(handles.stim_type,'type',handles.stim_type.String{1});
                                handles.stim_axis_confirm.String = handles.canalInfo.stimCanal{4};
                                case handles.canalInfo.stimNum{5}
                                setappdata(handles.stim_axis,'ax',handles.canalInfo.stimCanal{5});
                                handles.stim_axis.String = handles.canalInfo.stimCanal(5);
                                setappdata(handles.stim_type,'type',handles.stim_type.String{1});
                                handles.stim_axis_confirm.String = handles.canalInfo.stimCanal{5};
                        end
...
canalInfo.stimCanal = [eventdata.Source.Parent.Children(3).String(eventdata.Source.Parent.Children(3).Value),...
    ...
    eventdata.Source.Parent.Children(9).String(eventdata.Source.Parent.Children(9).Value),...
    eventdata.Source.Parent.Children(11).String(eventdata.Source.Parent.Children(11).Value)];
...
canalInfo.stimNum = [{eventdata.Source.Parent.Children(4).String(eventdata.Source.Parent.Children(4).Value)},...
    ...
    {eventdata.Source.Parent.Children(10).String(eventdata.Source.Parent.Children(10).Value)},...
    {eventdata.Source.Parent.Children(12).String(eventdata.Source.Parent.Children(12).Value)}]; 
...
%             toDel = find(~(a>(meanSize-300000) & a<(meanSize+300000)));
        ...
        Data.HeadMPUAccel_Z = zeros(length(Data.Time_Eye),1);
                        ...
                        temp(dashes(1)) = [];
                        if isempty(segments==0) || (segments==0)
                            handles.subj_id.String = {filename(dashes(3)+1:dashes(4)-1)};
                            handles.visit_number.String = {'NA'};
                            handles.date.String = temp(1:dashes(2)-2);
                            handles.exp_type.String = {'ElectricalStim'};
                            handles.exp_condition.String = {'PulseTrains'};
                            prompt = {'Enter the stimulation rate (pps):'};
                            t1 = 'Input';
...
%                             end
                        ...
                        if ~strcmp(prevStimCanal,getappdata(handles.stim_axis,'ax')) && (str2num(handles.segment_number.String)>1)
                            ...
                            setappdata(handles.stim_type,'type',filename(dashes(4)+1:amp-1));
                            handles.stim_type.String = {filename(dashes(4)+1:amp-1)};
                            setappdata(handles.stim_intensity,'intensity',filename(amp:dot-1));
                            handles.stim_intensity.String = {filename(amp:dot-1)};
                            [handles] = update_seg_filename(hObject, eventdata, handles);

%
% Name: voma__desaccadedata.m
%       - PJB       
%
% [desaccdata]=voma__desaccadedata(Data,time,Fs,derivVal,tSacc,tSaccMax,tSaccMin,splineVal,E,ThresholdQPRflag,Event,QP_range)
%
% Function: This routine was adapted to be a versatile tool for removing
% quick phases (QPs) from eye velocity recordings. The function first 
% performs spline smoothing on the data trace. Then, a point-by-point 
% difference of this data is taken and a threshold is set to mark saccadic 
% points. Additional parameters are used to combine individual saccades if 
% necessary. If two adjacent saccades are within a certain time interval, 
% the two saccades are combined. The data during the QP is deleted and a 
% cubic spline interpolation is applied between the edges.
%
% Input Parameters:
%       - Data
...
%       - derivVal
%           Value of the point-by-pont difference threshold for QP 
%           detection
%       - tSacc
%           If two QPs are within this time (in ms), they are combined into
%           one QP used for removal.
%       - tSaccMax (NOTE CURRENTLY USED. Left in for compatibility)
%           Maximum duration (in ms) of a QP. If a QP is longer than this, 
%           ignore it.
%       - tSaccMin(NOTE CURRENTLY USED. Left in for compatibility)
%           Maximum duration (in ms) of a QP. If a QP is shorter than this,
%           ignore it.
%       - splineVal
...
%           or No smoothing (0, else). Used to only run the algorithm on 
%           specific events
%       - ThresholdQPRflag
%           Binary flag indicating if the user wants the thresholding
%           algorithm to run ('true') or not ('false')
%       - Event (Optional)
%           n x 2 matrix, where the rows are number of 'events' and the 
%           columns are the start and stop indices of the events
%           indices that you
%       - QP_range (Optional)
%           n x 2 input array of QP indices
%           If added, the algorithm skips the QP detection section and used
%           the input array to blank out QPs
%
%
%   NOTE TO READER FROM PJB: Upon reviewing this code in 2018-05, I am
%   certain this routine can be properly optimized and improved. This
%   routine was developed as a amalgum of other peices of code and works 
%   sufficiently in its current state.
%
%   NOTE: This code was adapted from code inherited from Kristin
%   Hageman
%
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [desaccdata]=voma__desaccadedata(Data,time,Fs,derivVal,tSacc,tSaccMax,tSaccMin,splineVal,E,ThresholdQPRflag,Event,QP_range)
...
% Check if the user wants to do QP detection
if ThresholdQPRflag
    
    % Check if the user added a list of QP indices. If so, use that list
    % and don't run the detection algorithm.
    if ~exist('QP_range','var')
        
        
        
        % Pre-allocate space for a 'mask' to mark QPs
        findSac=zeros(1,length(DataSmth));
        
        % If the derivative is greater than "derivVal", then mark that point as a 'saccade'
        findSac([false ; abs(accel)>derivVal ]) = ones(length(findSac([false ; abs(accel)>derivVal ])),1);
        
        
        mask = [0; abs(accel)>derivVal];
        ...
        if isempty(QP_range)
            ...
            ind2remove = [];
            %reset QP_range values to be 6 ms before and after detected sacade index
...
for j=1:nnz(QP_range(:,1))
    ...
    % PJB: These slope terms are inaccurate. Using the 'maskedData'
    % variable causes the 'slope' input be zero for each QP. This seems to
    % work well for spline interpolation, but this is a clear situation 
    % where I will need to rewrite this routine to more accurately
    % determine the spline interpolation parameters.
